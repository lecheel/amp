use crate::errors::*;
use crate::models::application::modes::PendingDeleteMode;
...
pub fn display(
    ...
    let label = if pressed.is_empty() {
        ...
        format!("DELETE {}", pressed)
    };
    let status_entries = standard_status_line(&label, Colors::Warning, workspace, view, repo);
    ...
    let entries = presenter.which_key_pending_delete_entries(&mode.keys);
    if let Some(e) = error {
