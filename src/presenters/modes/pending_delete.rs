...
pub fn display(
    ...
    let status_entries = standard_status_line("DELETE", Colors::Warning, workspace, view, repo);
    ...
    let entries = presenter.which_key_entries("pending_delete");
    if let Some(e) = error {
        presenter.print_error(&e.to_string());
    } else {
        presenter.print_status_line(&status_entries);
    }
    presenter.print_which_key_popup("delete", &entries);
