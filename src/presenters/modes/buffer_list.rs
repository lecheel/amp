...
pub fn display(
    ...
    let modified_fg = RGBColor(0xE0, 0x8A, 0x68);
    let noname_fg = RGBColor(0x6C, 0x70, 0x86);
    ...
    let title_str = String::from(" Buffers ");
    ...
    for i in 0..FIXED_BODY_LINES {
        ...
        if result_index < results.len() {
            ...
            let is_no_name = entry.path.is_none();
            let display_str = entry.to_string();
            let (colors, style) = if result_index == selected {
                (Colors::Custom(sel_fg, sel_bg), Style::Bold)
            } else if is_no_name {
                (Colors::Custom(noname_fg, dark_bg), Style::Default)
            } else if entry.modified {
                (Colors::Custom(modified_fg, dark_bg), Style::Default)
            } else {
